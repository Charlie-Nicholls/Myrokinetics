...
class plot_diag(object):
	...
	def open_plot(self, save = False, filename = None):
		...
		self.fig.suptitle(scan['info']['run_name'])

		psiaxes = axes([0.25, 0.01, 0.5, 0.03])
		bpaxes = axes([0.25, 0.05, 0.5, 0.03])
		shaxes = axes([0.02, 0.25, 0.021, 0.5])

		self.psi_slider = Slider(psiaxes, 'psiN index:', 0, len(psiNs)-1, valinit = init[0], valstep = 1)
		self.psi_slider.on_changed(self.draw_fig)
		
		self.bp_slider = Slider(bpaxes, "-\u03B2' index:", 0, len(data['beta_prime_axis'][0])-1, valinit = init[1], valstep = 1)
		...
		if aky:
			...
			self.ky_slider = Slider(kyaxes, 'ky index:', 0, len(inputs['aky_values'])-1, valinit = init[3], valstep = 1,orientation = 'vertical')
	...
	def draw_fig(val = None):
		...
		if self.verify is not None:
			...
			if var == 1 and (psi_idx,bp_idx,sh_idx,aky_idx) in self.verify['phi']:
				bad.append('phi')
			if var == 2 and (psi_idx,bp_idx,sh_idx,aky_idx) in self.verify['apar']:
				bad.append('apar')
			if var == 3 and (psi_idx,bp_idx,sh_idx,aky_idx) in self.verify['apar']:
				bad.append('bpar')
			if bad:
				self.ax.text(0.01,0.01,f"BAD RUN: {str(bad)[1:-1]}",ha='left',va='bottom',transform=ax.transAxes,color='r')
