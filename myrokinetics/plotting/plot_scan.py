...
def plot_scan(self, aky = False):
	data = self.run['data']
	psiNs = self.run['inputs']['psiNs']
	if data['growth_rates'] is None:
		print("ERROR: No Gyrokinetic Data")
		return
	if aky and not data['growth_rates_all']:
		print("ERROR: Insufficent data")
	def draw_fig(val = None):
		...
		if aky:
			...
			ky = self.run['inputs']['aky_values'][ky_idx]
		...
		if options.get_status()[1]:
			ax[0].set_ylim(self.run['inputs']['shat_min'],self.run['inputs']['shat_max'])
			ax[0].set_xlim(bpmin,bpmax)
			ax[1].set_ylim(self.run['inputs']['shat_min'],self.run['inputs']['shat_max'])
	...
	fig.suptitle(self.run['info']['run_name'])
	...
	if aky:
		kys = len(self.run['inputs']['aky_values'])
		kyaxes = axes([0.15, 0.05, 0.5, 0.03])
		ky_slider = Slider(kyaxes, 'ky index:', 0, len(self.run['inputs']['aky_values'])-1, valinit = 0, valstep = 1)
		ky_slider.on_changed(draw_fig)
		
	slaxes = axes([0.15, 0.01, 0.5, 0.03])
	slider = Slider(slaxes, 'psiN index:', 0, len(psiNs)-1, valinit = 0, valstep = 1)
