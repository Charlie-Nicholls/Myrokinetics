...
class myro_set(object):
	...
	def _open_file(self, filename = None, directory = None):
		...
		try:
			...
			files = None
		self.runs = {'inputs': inputs, 'info': info, 'input_files': input_files, 'outputs': outputs, 'files': files}
		...
		for p, psiN in enumerate(psiNs):
			for k, aky in enumerate(values):
				for v, val in enumerate(akys):
					try:
						with open(f"{self.info['data_path']}/{psiN}/{k}_{v}.in") as in_file:
							in_lines[p][k][v] = in_file.readlines()
					...
					try:
						...
						out_dict[p][k][v] = None
_		file_lines = {'eq_file': self.eqbm._eq_lines, 'kin_file': self.eqbm._kin_lines, 'template_file': self._template_lines}
		savez(f"{self.path}/{filename}", inputs = self.inputs, run_info = self.info, input_files = in_lines, output_dicts = out_dict, files = file_lines)
